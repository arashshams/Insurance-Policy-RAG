...
**Windows:** Q2 uses the last 10 completed years. KPI series span 2016-2025; deaths span 2015-2024. Disclaimer (vintage): The exam is 2023-vintage. We keep the analysis faithful to a 2024 forecast target (train on <=2023) and expose a re-run flag in the script for an alternate window.
...
**How can the forecast be improved?** Several concrete steps: (1) **Correct for the 7-month coding lag** by using the 'Provincial overview - last year and year to date' dashboard to obtain within-year progress and apply a lag-development (claims-triangle / completion-factor) adjustment so provisional recent years are inflated to their expected final counts before fitting - this removes the systematic under-prediction we saw. (2) **Use a count model on the numerator with person-years as exposure** (Poisson / negative-binomial with log(person-years) offset, or a log-linear model on the rate) instead of OLS on the rate, which respects the non-negative, count nature of claims and stabilises the volatile severity series. (3) **Model the denominator (employment/person-years) separately** and combine, since the rate is a ratio - construction employment grew ~33% over the decade and drives the denominator. (4) **Widen and weight the training window** (e.g. exponentially weight recent years) and add a piecewise/segmented trend to capture the 2023-2024 flattening rather than forcing one straight line. (5) **Blend in leading/external signals** - construction employment outlook, permit/housing-start volumes, and inspection activity - and report a prediction interval (e.g. via bootstrap) rather than a point estimate, so the VP sees forecast uncertainty. (6) **Validate with rolling-origin back-tests** across multiple years, not a single 2024 hold-out, to get a more robust error estimate.
...
- Judgment calls: (i) sector-72 scope for deaths (more stable counts than subsector 7210 for rare events); (ii) OLS forecast method for transparency; (iii) 2024 as back-test target faithful to exam vintage; (iv) forecast-improvement recommendations (Q2b). All flagged for candidate ownership.
