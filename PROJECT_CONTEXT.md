...
## Paused — 2026-09-03 (live-demo bug hunt: truncated answers + a bad example question, FIXED)

Ran a live smoke test against the deployed Streamlit app (not just the pipeline in isolation) and found two real issues the earlier audits missed because they never exercised the actual shipped `app/demo_index/` through the actual UI copy:

1. **One of the three front-page example questions abstained.** "Are pre-authorizations required?" returned "I don't know" on the live app. Root-caused by pulling the real `app/demo_index/chroma.sqlite3` and reading its chunks directly: the phrase "pre-authorization" appears nowhere in the source policy — it only ever says **"Prior Authorization"** (see page 6, "When is a Prior Authorization form needed?"). The calibrated/eval-tested wording for this topic (`notebooks/eval/eval_questions.json`, `in_03`) is "When is a Prior Authorization form needed?" — the front-page button was a friendlier paraphrase written later for the README/UI (Day 7/8) and was never run through `04_evaluation.ipynb`. Combined with the already-documented ±1 chunk drift between the Colab calibration index (37 chunks) and the actually-shipped demo index (confirmed still 38 chunks), this untested paraphrase's cosine distance landed just over the 0.37 cutoff. A same-meaning rephrase using the document's own vocabulary ("Do I need prior approval before getting treatment?") retrieved correctly. Fix: changed the example button (and the matching README bullet) to "Is prior authorization required?" — close to the calibrated wording, still natural. Threshold/index were deliberately left untouched (that's a broader, separately-calibrated constant; not the right lever for one mis-worded example).

2. **Some answers were cut off mid-sentence** (e.g. "What expenses are excluded?", the prior-authorization rephrase). Root cause: `answer_question()` called Gemini's OpenAI-compatible endpoint with `max_tokens=512` and nothing else. On that endpoint, `gemini-flash-latest`'s internal "thinking" tokens are drawn from the *same* budget as the visible answer (not billed/capped separately), so a query that makes the model reason more can burn most of 512 tokens before it writes anything, truncating the answer — independent of `temperature=0.0`, and non-deterministic per question. Confirmed against Google's own OpenAI-compatibility docs and multiple independently-reported cases of the same failure mode on 2.5-Flash-family models. Fix in `src/rag_pipeline.py`: added `GEN_MAX_TOKENS` (default 1024, up from 512) and `GEN_REASONING_EFFORT` (default `"none"`, passed via `extra_body={"reasoning_effort": ...}` since the pinned `openai==1.14.3` client doesn't model that field natively but does support arbitrary `extra_body` passthrough — verified directly against the installed version). `answer_question()` now also reads `finish_reason` off the response and, if it's `"length"`/`"MAX_TOKENS"`, retries once with double the token budget; if still truncated after that, it appends an honest visible note ("this answer may have been cut off...") instead of silently shipping a partial sentence — in keeping with the project's whole "honest I-don't-know instead of a guess" ethos.
